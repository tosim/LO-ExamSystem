  ...
  <div>
    <div class="jbxx-all">
        ...
        <el-col :span="12">
          <div class="jbxx-t-r" style="margin:0 auto;float:left;height:200px;">
            <div style="line-height:150px;font-size:24px;text-align:center;">{{user.p_name}}
              <span style="color:red;font-style:italic">{{user.u_level}}</span>段
              <span style="color:rgb(255,193,0);font-style:italic">{{user.u_point}}</span>分
            </div>
      ...
      <div class="jbxx-title">
        ...
        <button class="layui-btn setting-btn" @click="userset" v-if="!saveuser&&!look">编辑</button>
      </div>
      <div class="jbxx-body">
        <div class="user-name">
      ...
      <div class="user-introduction">
        <div class="jbxx-title">
          <span>我的履历</span>
          <button class="layui-btn setting-btn" @click="showDialogResume()" v-if="!look">新建履历</button>
        </div>
...
export default {
  ...
  mounted: function () {
    ...
    // var self = this;
    console.log('at child :' + this.uid);
    var promise;
    if(this.uid){
      promise = this.$http.get(this.domain + '/v1/users/'+this.uid);
    }else{
      promise = this.$http.get(this.domain + '/v1/sessions')//1获取用户信息
    }
