      ...
      <el-table :data="wrong" style="width: 100%">
        ...
        <el-table-column label="查看" width="180">
          <template scope="scope">
            <el-button style="margin-top:2px;" class="layui-btn layui-btn-small" @click="showDetail(scope.row)">查看详情</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  mounted:function(){
    this.$http.get(this.domain+'/v1/sessions')//1获取用户信息
      ...
      .then(()=>{
        this.$http.get(this.domain+'/v1/u_que/tags?u_id='+this.user.u_id).then((res)=>{
          res = res.data;
          console.log(res.data);
          this.tags = res.data;
        })
      ...
      .then(()=>{
        ...
        this.$http.post(this.domain+'/v1/u_que',qs).then((res)=>{
          res = res.data;
          console.log(res);
